...
async def global_error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # CRITICAL FIX: Ignore Conflict/Network errors so the bot doesn't spam admins and freeze due to rate limiting
    ...
    try:
        ...
        # HTML Escape the error to prevent formatting crashes, truncate to keep it readable
        safe_error = html.escape(str(context.error))[:800]
        error_msg = f"🚨 <b>Bot Error Alert</b>:\n<blockquote><pre>{safe_error}</pre></blockquote>"
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .select-items { position: absolute; background-color: var(--card-bg); top: calc(100% + 4px); left: 0; right: 0; z-index: 999; border-radius: 10px; border: 5px solid var(--card-border); max-height: 180px; overflow-y: auto; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.5); }
        .select-items div { padding: 12px; cursor: pointer; color: #fff; font-size: 14px; border-bottom: 2px solid var(--card-border); transition: background 0.2s; }
...
<body>
        ...
        <div class="modal-content">
            ...
            <div class="filter-group">
                ...
                <div class="custom-select" id="filter-status" data-value="all">
                    <div class="select-selected" onclick="toggleCustomSelect(this)">All Status</div>
                    <div class="select-items select-hide">
                        <div onclick="setCustomSelect(this, 'all', 'All Status')">All Status</div>
    ...
    <script>
        ...
        function clearFilters() {
            ...
            resetSelect('filter-status', 'All Status');
...
def run_web_server():
    ...
    # CRITICAL FIX: Disabled reloader to ensure flask doesn't spawn an extra phantom process overriding polling
...
def main():
    ...
    # CRITICAL FIX: drop_pending_updates ignores backlog buildup clicks to prevent bot crashing
